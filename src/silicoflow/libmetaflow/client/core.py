...
from silicoflow.libmetaflow.environment import MetaflowEnvironment
from silicoflow.libmetaflow.exception import MetaflowNotFound,\
                               MetaflowNamespaceMismatch,\
                               MetaflowInternalError

from silicoflow.libmetaflow.metadata import LocalMetadataProvider, METADATAPROVIDERS
from silicoflow.libmetaflow.libmetaflow_config import DEFAULT_METADATA
from silicoflow.libmetaflow.plugins import ENVIRONMENTS

from silicoflow.libmetaflow.util import cached_property, resolve_identity
...
class MetaflowObject(object):
    ...
    def _get_object(self, *path_components):
        result = self._libmetaflow.metadata.get_object(self._NAME, 'self', None, *path_components)
        if not result:
    ...
    def __iter__(self):
        ...
        unfiltered_children = self._libmetaflow.metadata.get_object(
    ...
    def _get_child(self, id):
        ...
        for p in self.path_components:
            result.append(p)
        result.append(id)
        return self._libmetaflow.metadata.get_object(
...
class Task(MetaflowObject):
    ...
    def metadata(self):
        ...
        """
        all_metadata = self._libmetaflow.metadata.get_object(
