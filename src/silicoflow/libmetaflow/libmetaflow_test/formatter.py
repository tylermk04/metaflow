...
class FlowFormatter(object):
    ...
    def _flow_lines(self):
        ...
        yield 0, 'from silicoflow.libmetaflow import FlowSpec, step, Parameter, JSONType'
        yield 0, 'from silicoflow.libmetaflow.libmetaflow_test import assert_equals, '\
                                           'assert_exception, '\
                                           'ExpectationFailed, '\
                                           'is_resumed, '\
                                           'ResumeFromHere, '\
                                           'TestRetry'
        if tags:
            yield 0, 'from silicoflow.libmetaflow import %s' % ','.join(tags)
    ...
    def _check_lines(self):
        ...
        yield 0, 'cov.start()'
        yield 0, 'import sys'
        yield 0, 'from silicoflow.libmetaflow.libmetaflow_test import assert_equals, new_checker'
