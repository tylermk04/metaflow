...
from silicoflow.libmetaflow.datastore import MetaflowDataStore
from silicoflow.libmetaflow.datastore.local import LocalDataStore
from silicoflow.libmetaflow.datastore.util.s3util import get_s3_client
from silicoflow.libmetaflow.libmetaflow_config import DATASTORE_LOCAL_DIR
from silicoflow.libmetaflow import util
from silicoflow.libmetaflow.exception import (
    CommandException,
...
def step(
    ...
    **kwargs
    ...
    attrs = {
        "libmetaflow.user": util.get_username(),
        "libmetaflow.flow_name": ctx.obj.flow.name,
        "libmetaflow.step_name": step_name,
        "libmetaflow.run_id": kwargs["run_id"],
        "libmetaflow.task_id": kwargs["task_id"],
        "libmetaflow.retry_count": str(retry_count),
        "libmetaflow.version": ctx.obj.environment.get_environment_info()[
            "libmetaflow_version"
        ],
    }
    ...
    batch = Batch(ctx.obj.metadata, ctx.obj.environment)
    try:
        with ctx.obj.monitor.measure("libmetaflow.batch.launch"):
            batch.launch_job(
                step_name,
                step_cli,
                code_package_sha,
