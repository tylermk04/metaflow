...
from silicoflow.libmetaflow.datastore.local import LocalDataStore
from silicoflow.libmetaflow.decorators import StepDecorator
from silicoflow.libmetaflow.environment import InvalidEnvironmentException
from silicoflow.libmetaflow.metadata import MetaDatum
from silicoflow.libmetaflow.libmetaflow_config import get_pinned_conda_libs, CONDA_PACKAGE_S3ROOT
from silicoflow.libmetaflow.util import get_libmetaflow_root
from silicoflow.libmetaflow.datatools import S3
...
class CondaStepDecorator(StepDecorator):
    ...
    def _env_id(self):
        ...
        return 'libmetaflow_%s_%s_%s' % (self.flow.name, 
                                        self.architecture, 
    ...
    def runtime_init(self, flow, graph, package, run_id):
        # Create a symlink to installed version of libmetaflow to execute user code against
        path_to_libmetaflow = os.path.join(get_libmetaflow_root(), 'libmetaflow')
        self.libmetaflow_home = tempfile.mkdtemp(dir='/tmp')
        os.symlink(path_to_libmetaflow, os.path.join(self.libmetaflow_home, 'libmetaflow'))
    ...
    def runtime_step_cli(self, cli_args, retry_count, max_user_code_retries):
        if self.is_enabled() and 'batch' not in cli_args.commands:
            python_path = self.libmetaflow_home
    ...
    def runtime_finished(self, exception):
        shutil.rmtree(self.libmetaflow_home)