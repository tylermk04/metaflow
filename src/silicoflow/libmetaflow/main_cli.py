...
from .libmetaflow_config import DATASTORE_LOCAL_DIR, DEFAULT_METADATA
...
def main(ctx):
    ...
    from silicoflow import libmetaflow
    echo('Metaflow ',
         fg='magenta',
         bold=True,
         nl=False)
    ...
    if ctx.invoked_subcommand is None:
        echo('(%s): ' % libmetaflow.__version__,
             fg='magenta',
             bold=False,
             nl=False)
    else:
        echo('(%s)\n' % libmetaflow.__version__,
             fg='magenta',
             bold=False)
    ...
    if ctx.invoked_subcommand is None:
        ...
        # libmetaflow URL
        echo('http://docs.libmetaflow.org', fg='cyan', nl=False)
        echo(' - Read the documentation')

        # libmetaflow chat
        echo('http://chat.libmetaflow.org', fg='cyan', nl=False)
        echo(' - Chat with us')

        # libmetaflow help email
        echo('help@libmetaflow.org', fg='cyan', nl=False)
        echo('        - Get help by email\n')

        # print a short list of next steps.
        short_help = {'tutorials': 'Browse and access libmetaflow tutorials.',
                      'configure': 'Configure libmetaflow to access the cloud.',
        ...
        for cmd, desc in short_help.items():
            echo('  libmetaflow {0:<10} '.format(cmd),
                 fg='cyan',
                 bold=False,
                 nl=False)
...
def status():
    ...
    if res[0] == 'service':
        ...
        echo('1. python')
        echo('2. from silicoflow.libmetaflow import Metaflow')
        echo('3. list(Metaflow())')
        return

    from .client import namespace, metadata, Metaflow
...
@main.group(help="Browse and access the libmetaflow tutorial episodes.")
def tutorials():
    pass

def get_tutorials_dir():
    libmetaflow_dir = os.path.dirname(__file__)
    package_dir = os.path.dirname(libmetaflow_dir)
    tutorials_dir = os.path.join(package_dir, 'libmetaflow', 'tutorials')
...
def list():
    ...
    echo('libmetaflow tutorials pull', fg='cyan')
...
def validate_episode(episode):
    ...
    if not os.path.isdir(src_dir):
        raise click.BadArgumentUsage("Episode " + \
                                     ...
                                     "type:\n" + \
                                     click.style("libmetaflow tutorials list",
                                                 fg='cyan'))
...
def pull(episode):
    ...
    # Create destination `libmetaflow-tutorials` dir.
    dst_parent = os.path.join(os.getcwd(), 'libmetaflow-tutorials')
    makedirs(dst_parent)
    ...
    for episode in episodes:
        ...
        # Copy from (local) libmetaflow package dir to current.
        src_dir = os.path.join(tutorials_dir, episode)
        shutil.copytree(src_dir, dst_dir)

    echo('\nTo know more about an episode, type:\n', nl=False)
    echo('libmetaflow tutorials info [EPISODE]', fg='cyan')
...
# NOTE: This code needs to be in sync with libmetaflow/libmetaflow_config.py.
METAFLOW_CONFIGURATION_DIR =\
    expanduser(os.environ.get('METAFLOW_HOME', '~/.libmetaflowconfig'))
...
@configure.command(help='Configure libmetaflow to access hosted sandbox.')
@click.option('--profile', '-p', default='',
...
@configure.command(help='Configure libmetaflow to access self-managed AWS resources.')
@click.option('--profile', '-p', default='',
                help='Configure a named profile. Activate the profile by setting ' 
                    '`METAFLOW_PROFILE` environment variable.')
def aws(profile):
    ...
    if not click.confirm('Already configured ' + cyan('AWS access credentials') + '?',
        ...
        if use_s3:
            ...
            env_dict['METAFLOW_DATASTORE_SYSROOT_S3'] =\
                click.prompt('\t' + cyan('METAFLOW_DATASTORE_SYSROOT_S3:') + 
                  ' Amazon S3 url for libmetaflow datastore (s3://<bucket>/<prefix>)')

            env_dict['METAFLOW_DATATOOLS_SYSROOT_S3'] =\
                click.prompt('\t' + cyan('METAFLOW_DATATOOLS_SYSROOT_S3:') + 
                  yellow(' (optional)') + ' Amazon S3 url for libmetaflow datatools (s3://<bucket>/<prefix>)',
            ...
            if use_batch:
                ...
                libmetaflow_batch_container_registry =\
                ...
                if libmetaflow_batch_container_registry:
                    env_dict['METAFLOW_BATCH_CONTAINER_REGISTRY'] = libmetaflow_batch_container_registry
                libmetaflow_batch_container_image =\
                    click.prompt('\t' + cyan('METAFLOW_BATCH_CONTAINER_IMAGE:') + 
                      yellow(' (optional)') + ' Default docker image for AWS Batch jobs',
                        default='', show_default=False)
                if libmetaflow_batch_container_image:
                    env_dict['METAFLOW_BATCH_CONTAINER_IMAGE'] = libmetaflow_batch_container_image
        ...
        if use_metadata:
            ...
            libmetaflow_service_auth_key =\
                click.prompt('\t' + cyan('METAFLOW_SERVICE_AUTH_KEY:') + 
                  yellow(' (optional)') + ' Auth key for Metadata Service',
                    default='', show_default=False)
            if libmetaflow_service_auth_key:
                    env_dict['METAFLOW_SERVICE_AUTH_KEY'] = libmetaflow_service_auth_key
        persist_env(env_dict, profile)
