...
from ..libmetaflow_config import DATATOOLS_S3ROOT
...
from ..libmetaflow_config import get_authenticated_boto3_client
...
class S3(object):
    ...
    def __init__(self,
        ...
        self._tmpdir = mkdtemp(dir=tmproot, prefix='libmetaflow.s3.')
            ...
            for key, obj in key_objs:
                if is_stringish(obj):
                    with NamedTemporaryFile(dir=self._tmpdir,
                                            delete=False,
                                            mode='wb',
                                            prefix='libmetaflow.s3.put_many.') as tmp:
    ...
    def _one_boto_op(self, op, url):
        ...
        for i in range(NUM_S3OP_RETRIES):
            tmp = NamedTemporaryFile(dir=self._tmpdir,
                                     prefix='libmetaflow.s3.one_file.',
                                     delete=False)
            try:
    ...
    def _read_many_files(self, op, prefixes, **options):
        with NamedTemporaryFile(dir=self._tmpdir,
                                ...
                                prefix='libmetaflow.s3.inputs.') as inputfile:
    ...
    def _put_many_files(self, url_files, overwrite):
        ...
        with NamedTemporaryFile(dir=self._tmpdir,
                                mode='wb',
                                delete=not debug.s3client,
                                prefix='libmetaflow.s3.put_inputs.') as inputfile:
    ...
    def _s3op_with_retries(self, mode, **options):
        ...
        for i in range(NUM_S3OP_RETRIES):
            with NamedTemporaryFile(dir=self._tmpdir,
                                    mode='wb+',
                                    delete=not debug.s3client,
                                    prefix='libmetaflow.s3op.stderr') as stderr:
                try:
