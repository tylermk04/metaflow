...
from silicoflow.libmetaflow.libmetaflow_test import MetaflowTest, ExpectationFailed, steps, tag
...
class TagCatchTest(MetaflowTest):
    ...
    def step_end(self):
        from silicoflow.libmetaflow.exception import ExternalCommandFailed
    ...
    def check_results(self, flow, checker):
        ...
        for step in flow:
            ...
            if step.name == 'start':
                ...
                try:
                    for task in checker.artifact_dict('start',
                                                      'invisible').values():
                        if task:
                            raise Exception("'invisible' should not be visible "\
                                            "in 'start'")
                except KeyError:
                    pass
            elif step.name == 'end':
                ...
                for task in checker.artifact_dict(step.name, 'ex').values():
                    extype = 'libmetaflow.plugins.catch_decorator.'\
        ...
        if run:
            for step in run:
                ...
                for task in step:
                    data = task.data
                    got = sorted(m.value for m in task.metadata
                                 if m.type == 'attempt')
            ...
            assert_equals('libmetaflow.exception.ExternalCommandFailed',
                          end.data.end_ex.type)
