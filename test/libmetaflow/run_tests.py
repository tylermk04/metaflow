...
from silicoflow.libmetaflow.libmetaflow_test import MetaflowTest
from silicoflow.libmetaflow.libmetaflow_test.formatter import FlowFormatter
...
def iter_tests():
    ...
    for testfile in os.listdir(root):
        if testfile.endswith('.py') and not testfile[0] == '.':
            mod = importlib.import_module(testfile[:-3], 'libmetaflow_test')
            for name in dir(mod):
                obj = getattr(mod, name)
                if name != 'MetaflowTest' and\
                   isinstance(obj, type) and\
                   issubclass(obj, MetaflowTest):
                    yield obj()
...
def run_test(formatter, context, coverage_dir, debug, checks):
    ...
    cwd = os.getcwd()
    tempdir = tempfile.mkdtemp('_libmetaflow_test')
    ...
    try:
        ...
        if 'pre_command' in context:
            if context['pre_command'].get('libmetaflow_command'):
            ...
            if pre_ret and not context['pre_command'].get('ignore_errors',
                                                          False):
        ...
        for check_name in context['checks']:
            ...
            if check_ret:
                ...
                log("checker '%s' says that results are ok" % check_name,
                    formatter,
                    context)
...
def run_test_cases(args):
    ...
    if os.environ.get('METAFLOW_TEST_RUNNER', '') == 'jenkins':
        ...
        contexts = json.load(open(Path(__file__).parent / 'contexts.json'))
...
@click.option("--num-parallel",
              ...
              default=None,
              type=int,
              help="Number of parallel tests to run. By default, "
...
def cli(tests=None,
    ...
    coverage_dir = tempfile.mkdtemp('_libmetaflow_test_coverage') if coverage_output else None
    try:
        failed = run_all(parse(tests),
                         parse(contexts),
                         parse(graphs),
                         coverage_dir,
                         debug,
        ...
        if coverage_output and not debug:
            produce_coverage_report(coverage_dir,
                                    os.path.abspath(coverage_output))
